...
# %% load image data
all_dataset = load_real_samples('data256_x100.npz')
dataset = all_dataset[0][:500], all_dataset[1][:500]
# %%
image_shape = dataset[0].shape
src, tar = dataset[0].reshape((*image_shape, 1)), dataset[1].reshape(
    (*image_shape, 1))
srctar = list(zip(src, tar))
from random import shuffle
shuffle(srctar)
src, tar = list(zip(*srctar))
import numpy as np
src = np.array(src)
tar = np.array(tar)
#[:500] # training data
...
# %% train model
train(d_model, g_model, gan_model, (src, tar))
